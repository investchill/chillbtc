...
Cette page explique **pourquoi** la stratégie utilise exactement 2 signaux (R1
tendance et R3 valorisation), **comment** chacun est construit, et **quand**
ils se sont avérés utiles ou coûteux sur l'historique BTC. Le corps est
accessible à tout lecteur intéressé. Les encadrés *Pour l'expert* donnent les
formules, les citations DOI et les limites académiques.
...
Un signal unique reste prisonnier d'un régime de marché. La tendance (R1)
fonctionne bien quand le prix se déplace franchement dans une direction, mais
rate les retournements en zone euphorique. La valorisation (R3) repère ces
zones euphoriques, mais reste muette quand le prix traîne à mi-chemin de sa
trajectoire long-terme.

ChillBTC combine donc deux signaux **orthogonaux** : l'un regarde la dérivée
du prix (tendance), l'autre regarde le niveau absolu par rapport à une droite
de référence (valorisation). Quand l'un des deux dit *CASH*, la position est
allégée ou coupée, sans attendre que le second s'en rende compte.
...
## 2. R1 — Time-Series Momentum

R1 répond à une question simple : **le prix BTC a-t-il monté sur les 11
derniers mois** ? Si oui, la tendance est favorable et R1 dit BUY. Sinon, R1
dit CASH.
...
La fenêtre n = 11 a été retenue par optimisation O2 (walk-forward) sur
l'axe minimisation du *max drawdown*. Ce n'est pas un pic isolé : le plateau
de stabilité couvre n ∈ {9, 10, 11, 12, 13}, tous produisant un DD voisin. Un
paramètre choisi dans un plateau large résiste mieux aux régimes hors-
échantillon qu'un pic étroit.

> **Pour l'expert.** R1 est implémenté comme
...
Trois moments charnières où R1 a pesé :

- **2018-09** : R1 bascule CASH après un retard de plusieurs mois sur le top
  de janvier. La stratégie sort avant la chute de Q4 2018.
- **2020-03** : R1 reste CASH après le krach Covid. La stratégie rate le
  rebond en V. C'est le coût du lag TSMOM, assumé par construction.
- **2022-04 à 2022-08** : R1 en CASH tout au long de la descente Luna /
  3AC / FTX. La stratégie traverse le bear à 0 % ou 50 %.
...
R3 répond à une autre question : **le prix BTC est-il cher ou pas cher par
rapport à sa trajectoire long-terme** ? La référence est une droite en
log-log : `log P = A + N × log(days_since_genesis)`, avec N = 5.8 fixé et A
recalibré une fois par an au 1ᵉʳ janvier, puis gelé le reste de l'année.

Quand le prix s'écarte fortement au-dessus de la droite, R3 bascule CASH.
Quand le prix repasse nettement en dessous, R3 rebascule BUY. Entre les deux,
R3 maintient son état précédent.
...
- **2017-12** : `close/fair` dépasse 4. R3 CASH au pic. La stratégie coupe
  juste avant le krach de 2018.
- **2021-11** : R3 CASH dès l'été 2021, tenu jusqu'au bottom 2022. La
  stratégie évite l'essentiel de la baisse.
- **2023-01** : `close/fair < 0.6` au bottom à 17 000 USD. R3 repasse BUY
  tôt, captant une partie du rebond vers 23 000 USD en janvier.
...
- R1 BUY et R3 BUY, 100 % BTC, climat haussier assumé
- R1 BUY et R3 CASH, 0 % BTC, R3 CASH prime (edge case strict_r1_def)
- R1 CASH et R3 BUY, 50 % BTC + 50 % USDC, tendance floue
- R1 CASH et R3 CASH, 0 % BTC + 100 % USDC, bear confirmé
...
- **2017** bull : R3 CASH dès Q4, R1 encore BUY. Position à 50 % au pic.
- **2018** bear : R1 et R3 CASH simultanément. Position à 0 % pendant l'essentiel
  du crash.
- **2021** cycle : R1 BUY presque toute l'année, R3 CASH dès l'été. Position à
  50 % au top de novembre.
- **2022** FTX : R1 et R3 CASH avant l'effondrement. Position à 0 % en novembre.
...
**Garde-fous en place.** Optimisation par walk-forward (O2), validation par
leave-one-cycle-out (O3), plateau de stabilité exigé sur les paramètres, gel
annuel de A, journal mensuel obligatoire pour détecter la dérive. Ce sont
des filets, pas des certitudes. Le chapitre
[`CONTRIBUTING.md`](../CONTRIBUTING.md) détaille le scope verrouillé anti-overfit.
...
confirment qu'aucune IA ne figure comme autrice. Conforme aux politiques
ACM Authorship 2023 et Elsevier Generative AI Policy 2023.
