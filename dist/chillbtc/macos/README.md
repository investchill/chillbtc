...
### Étape 2 — Lancer `chill.command`

Ouvre le dossier `macos/` dans le Finder. Tu y vois :

- `README.md` (ce fichier)
- `chill.command` ← **le launcher**

**Double-clique sur `chill.command`.**

### Étape 3 — Passer le Gatekeeper (première fois uniquement)

macOS va probablement afficher une alerte du genre :

> *"chill.command" n'a pas pu être ouvert car Apple ne peut pas vérifier
> qu'il ne contient pas de logiciel malveillant.*

C'est normal : le fichier vient d'une source hors App Store. Pour l'autoriser :

1. Ferme l'alerte.
2. **Clic droit** sur `chill.command` → **Ouvrir**.
3. macOS propose maintenant **Ouvrir** dans la boîte de dialogue : clique dessus.
4. Une fenêtre Terminal s'ouvre avec le script qui démarre.

Tu n'auras à faire ce clic droit **qu'une seule fois**. Les lancements
...
toute première fois.** Tu vois les barres de progression dans le terminal.
...
1. Double-clique sur `chill.command`.
...
4. **Si la position change**, ajuste ton allocation sur la plateforme avec
   ...
   - 50 % → moitié BTC, moitié USDC (au mieux-
     disant).
   - 0 % → tout en USDC.
5. Ferme le terminal. Reviens dans un mois.
...
**Q : Je veux tester sur un autre Mac ou transférer l'outil.**
...
`chill.command`. La première install sera refaite (30 s à 2 min).
...
aussi le binaire `uv` (`~/.local/bin/uv`) et le cache uv
(`~/.cache/uv/`) si tu ne t'en sers pas pour d'autres projets.

**Q : Pourquoi le dossier `code/` n'est pas dans `macos/` ?**

R : `code/` contient le moteur Python partagé entre macOS et Windows. Le
séparer évite la duplication. Le launcher `chill.command` sait aller le
